...
url_set = set()
with open("/raid/urls/all_urls_unique.txt", "r") as fr:
    for line in fr:
        line = json.loads(line)
        if not line.strip() == "" and line.startswith('http'):
            url_set.add(str(line.strip()))

out_file = open("/raid/train_data_for_urls.txt", "w")
...
for url in url_set:
    ...
    # Handle the case for missing training '/' in a web url for the cases only where the url was not found in the index
    ...
    if data['info'] and not data['info'] is None:
        ...
        url_words = " ".join(du.words_from_url(url_tmp, order=True))
        try:
            ...
            print "Exception encountered for url: {}, exception: {}".format(url_tmp, e)
